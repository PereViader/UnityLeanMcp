...
- **Solution**: Avoid static shared result files. Use operation-scoped unique result paths (`Temp/unity_<kind>_<opId>.json`) and atomically move temporary files into place via `File.Move(tempPath, targetPath)`. Moving to an uncreated path avoids `ReplaceFileW` and requires no exclusive replacement locks. For files that are repeatedly overwritten (such as `unity_operation.json`), use platform-native move-overwrite (`MoveFileEx` / `rename`) via `UnityLeanMcpStaticHistoryWriter.MoveWithOverwrite` inside `WriteAtomic` instead of `File.Replace`.
