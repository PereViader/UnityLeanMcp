...
- **Type-Safe Result Path Resolution**: `IUnityPathResolver` and `UnityLeanMcpPaths` avoid per-command property proliferation (`RefreshResultFile`, `EvalResultFile`, `TestResultsFile`, `GetEvalResultFile`, `GetTestResultsFile`) by consolidating result file resolution into a single method: `string GetResultFilePath(UnityOperationKind kind, string? operationId = null)` in the MCP layer, and `string GetResultFilePath(string kind, string operationId = null)` in the Editor package.
...
- **Unified Operation Polling Spec**: All mutating operation polling is driven through `IOperationPoller.PollOperationUntilTerminalAsync` configured via declarative `OperationPollingSpec<TResult>` records, eliminating redundant legacy polling overloads and specialized helper wrappers.
...
- **Anonymous Object Return Pattern for Property Inspection**: When querying or inspecting multiple fields or properties on target objects, returning an anonymous object (e.g., `return new { player.health, player.speed };`) is the recommended pattern. It allows callers and AI agents to query structured data concisely in a single evaluation round-trip without declaring custom data transfer classes.
...
- **Actionable Missing-Namespace Hints**: When dynamic snippet compilation fails with `CS0103` (symbol not found) or `CS0246` (type not found) referencing common Unity types or APIs (`GameObject`, `Transform`, `Vector3`, `Selection`, `AssetDatabase`, etc.), `unity_eval` appends an actionable hint (`Hint: Missing using directive? Include 'using UnityEngine;' or 'using UnityEditor;' at the top of your snippet.`) to its failure trailer so AI agents and users immediately recognize that explicit namespace imports are needed.
- **Separation of Concerns in Tool Schemas**: The tool description defines the complete execution contract (statements, await, return rules including the anonymous object return pattern, using directives), while the parameter description remains strictly focused on text representation (plain text, avoiding JSON wrapping).
...
- Built-in type formatters are modularized: `TransformFormatter` (Priority 110), `GameObjectFormatter` (Priority 100), `ComponentFormatter` (Priority 90), `ScriptableObjectFormatter` (Priority 80), `SceneFormatter` (Priority 70), `SerializedObjectFormatter` (Priority 60), `SerializedPropertyFormatter` (Priority 50), `EnumerableFormatter` (Priority 40), `UnityMathFormatter` (Priority 35), and `JsonUtilityFallbackFormatter` (Priority -1000).
- `UnityMathFormatter` provides high-density, concise invariant culture representations for common Unity math and geometric value types (`Vector2`, `Vector3`, `Vector4`, `Vector2Int`, `Vector3Int`, `Quaternion`, `Color`, `Color32`, `Bounds`, `BoundsInt`, `Rect`, `RectInt`, `Ray`, `Ray2D`, `Plane`) using fixed-point formatting (e.g. `F2` or `F3`), preventing token bloat and avoiding verbose multi-line JSON fallback serialization.
...
- **String Enum Schema**: `mode` uses the `UnityTestMode` enum with JSON string values `editmode` and `playmode`. The ambiguous `all` mode is eliminated because it does not reliably discover PlayMode integration tests. Schema-aware clients reject unsupported modes before dispatch. An explicit invalid enum value or legacy `all` is rejected at the tool boundary with a clear diagnostic directing callers to choose `editmode` or `playmode`.
- **Mandatory Test Mode & Actionable Guidance**: `mode` is a required parameter without a default value, requiring callers and AI agents to determine whether target tests belong to EditMode (fast unit tests, editor utilities, no player/runtime lifecycle) or PlayMode (integration tests requiring scene loading, MonoBehaviour lifecycle, or frame yielding via `UnityTest`/`IEnumerator`). When a test run returns zero matching tests for a filter, diagnostic guidance suggests the opposite mode to quickly recover from mode mismatches.
- **Selective Failure Re-Run Guidance (`failedOnly`)**: The `failedOnly` parameter and `unity_run_tests` tool descriptions guide AI agents to use `failedOnly` selectively—re-running only failed tests when test suites take a long time to complete, while skipping it when tests run quickly to keep iterations straightforward.

### Test Mode Validation
The public `unity_run_tests` tool requires callers to explicitly specify `editmode` or `playmode`, exposes only those accepted string enum values, and rejects `all` and invalid enum values before refreshing or dispatching work to Unity. When callers supply the removed `all` mode, validation returns an explicit diagnostic message explaining why `all` was removed and guiding them to select `editmode` or `playmode`. The lower-level socket protocol continues to validate its string mode contract independently.
