...
- **Single-Pass Using Directive Extraction in Snippet Evaluation**: `EvalHandler.TryCompileSnippet` extracts using directives once and passes `alreadyCleaned: true` to `RoslynCompilerHelper.HasTopLevelValueReturn(methodBody, alreadyCleaned: true)`. This eliminates redundant AST parsing and regex fallback passes over already-sanitized snippet bodies.
- **Streamlined Foreign Lock Detection (`IsForeignOperationActive`)**: In `UnityClient.WaitForActiveOperationGracePeriodAsync`, foreign active operation evaluation across wire protocol busy responses and durable operation store fallbacks is unified via `IsForeignOperationActive`. It cleanly checks busy status, compilation immunity, and case-insensitive matching for operation kind and ID, eliminating duplicated conditional expressions and ensuring symmetric lock clearance logic.
- **Pre-Cached Editor LogEntries Reflection Members & Deduplicated Diagnostics Snapshots**: In `UnityLeanMcpCompilationTracker`, reflection handles for `UnityEditor.LogEntries` / `LogEntry` (`GetCount`, `GetEntryInternal`, `StartGettingEntries`, `EndGettingEntries`, `Clear`, and fields `condition`, `errorNum`, `file`, `line`, `column`, `mode`) are resolved once during main-thread initialization and cached for the domain lifetime, eliminating repeated reflection scans on every active error sweep. Diagnostics lock acquisition and list aggregation across assembly compilations are encapsulated in `GetCapturedDiagnosticsSnapshot()`, deduplicating snapshot creation across compilation callbacks and error sweeps.
- **Direct Domain Service Access & O(N) Test Failure Deduplication**: `RunTestsHandler` eliminates redundant static forwarding properties and methods (`TempDirectory`, `RunningFilePath`, `CancellationFilePath`, `ResultsFilePath`, `GetResultsFilePath`, `WriteAtomic`, `TryWriteStaticHistory`) in favor of direct calls to `UnityLeanMcpPaths` and `UnityLeanMcpOperationStore`. `GetPreviouslyFailedTestNames` replaces $O(N^2)$ `List.Contains` linear scans with `HashSet<string>` configured with `StringComparer.Ordinal` for $O(N)$ accumulation and deduplication.
